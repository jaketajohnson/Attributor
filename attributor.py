...
def Attributor():
    ...
    manhole_exception = "(WATERTYPE = 'SW' or OWNEDBY = -2) And FACILITYID IS NULL"
    ...
    week_ago = week_ago.strftime(r"%m/%d/%Y %H:%M:%S %p")
        ...
        for asset in sewer_assets:
            ...
            if int(arcpy.GetCount_management(selection_by_date).getOutput(0)) > 0:
                ...
                if asset[0] == sewer_manhole:
                    selection = arcpy.SelectLayerByAttribute_management("asset_temp", "NEW_SELECTION", manhole_exception)
    ...
    def ward_attribution():
        """Attribute the sewer main GXPCity field using the administrative area polygon's label field its center is in. If it's a ward, add text to the label."""
